...
pub fn dotprod_i32(sum: I32Vec, x: U8Vec, y: I8Vec) -> I32Vec {
    ...
    unsafe {
        _mm256_dpbusd_avx_epi32(sum, x, y)
    }
    ...
    unsafe {
        ...
        let hi = _mm256_set1_epi8(0x80u8 as i8);
        let s1 = _mm256_maddubs_epi16(_mm256_and_si256(x, hi), y);
        let s2 = _mm256_maddubs_epi16(_mm256_andnot_si256(hi, x), y);
        let s1 = _mm256_madd_epi16(s1, ones);
        let s2 = _mm256_madd_epi16(s2, ones);
        _mm256_add_epi32(sum, _mm256_add_epi32(s1, s2))
    }
}
...
pub fn reduce_add_f32(v: F32Vec) -> f32 {
    unsafe {
        let sum_128 = _mm_add_ps(_mm256_castps256_ps128(v), _mm256_extractf128_ps::<1>(v));
        let sum_64 = _mm_add_ps(sum_128, _mm_movehl_ps(sum_128, sum_128));
        _mm_cvtss_f32(_mm_add_ss(sum_64, _mm_shuffle_ps::<1>(sum_64, sum_64)))
    }
}

#[allow(clippy::cast_sign_loss)]
pub fn nonzero_mask_i32(v: I32Vec) -> Mask32 {
    let zeros = unsafe { _mm256_movemask_ps(_mm256_castsi256_ps(_mm256_cmpeq_epi32(v, _mm256_setzero_si256()))) };
    !(zeros as Mask32) & ((1 << I32_LANES) - 1)
}
