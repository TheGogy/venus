...
const fn next_rng(state: u64) -> u64 {
    let mut state = state;
    state ^= state >> 12;
    state ^= state << 25;
    state ^= state >> 27;
...
const CASTLING: [u64; CastlingRights::NUM] = {
