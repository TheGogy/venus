...
impl Board {
    pub fn gen_moves<MT: MGType>(&self) -> MoveList {
        let mut ml = MoveList::default();
        self.enumerate_moves::<MT>(&mut ml);
        ml
    }

    pub fn enumerate_moves<MT: MGType>(&self, ml: &mut MoveList) {
        match self.state.checkers.nbits() {
            // No checkers: enumerate all moves.
            0 => {
                self.enumerate_pawn::<MT, false>(ml);
                self.enumerate_knight::<MT, false>(ml);
                self.enumerate_diag::<MT, false>(ml);
                self.enumerate_orth::<MT, false>(ml);
                self.enumerate_king::<MT, false>(ml);
            }

            // 1 checker: enumerate all moves in the checkmask.
            1 => {
                self.enumerate_pawn::<MT, true>(ml);
                self.enumerate_knight::<MT, true>(ml);
                self.enumerate_diag::<MT, true>(ml);
                self.enumerate_orth::<MT, true>(ml);
                self.enumerate_king::<MT, true>(ml);
            }
    ...
    #[inline]
    fn add_moves<MT: MGType, const CHECK: bool>(&self, from: Square, mut dest: Bitboard, ml: &mut MoveList) {
        if CHECK {
    ...
    #[inline]
    fn add_promos<MT: MGType>(&self, src: Square, tgt: Square, ml: &mut MoveList) {
        if MT::NOISY {
    ...
    /// Add all pawn moves.
    #[inline]
    fn enumerate_pawn<MT: MGType, const CHECK: bool>(&self, ml: &mut MoveList) {
        ...
        if !promo.is_empty() {
            ...
            bb.bitloop(|s| {
                self.add_promos::<MT>(s.sub_dir(up), s, ml);
            });
        }
    ...
    /// Add all king moves.
    #[inline]
    fn enumerate_king<MT: MGType, const CHECK: bool>(&self, ml: &mut MoveList) {
        let ksq = self.ksq(self.stm);

        // Generate regular king moves (assume not in check: we want to stay out of the checkmask!)
        self.add_moves::<MT, false>(ksq, king_attacks(ksq) & !self.state.attacked, ml);
    ...
    #[inline]
    fn enumerate_knight<MT: MGType, const CHECK: bool>(&self, ml: &mut MoveList) {
        ...
        knights.bitloop(|s| {
            self.add_moves::<MT, CHECK>(s, knight_attacks(s) & !self.c_bb(self.stm), ml);
        });
    }

    /// Add all diagonal slider moves.
    #[inline]
    fn enumerate_diag<MT: MGType, const CHECK: bool>(&self, ml: &mut MoveList) {
        ...
        (diag & !self.state.pin_diag).bitloop(|s| {
            self.add_moves::<MT, CHECK>(s, bishop_attacks(s, occ) & ok, ml);
        });

        // Pinned bishop + queen.
        (diag & self.state.pin_diag).bitloop(|s| {
            self.add_moves::<MT, CHECK>(s, bishop_attacks(s, occ) & ok & self.state.pin_diag, ml);
        });
    }

    // Add all orthogonal slider moves.
    #[inline]
    fn enumerate_orth<MT: MGType, const CHECK: bool>(&self, ml: &mut MoveList) {
        ...
        (orth & !self.state.pin_orth).bitloop(|s| {
            self.add_moves::<MT, CHECK>(s, rook_attacks(s, occ) & ok, ml);
        });

        // Pinned rook + queen.
        (orth & self.state.pin_orth).bitloop(|s| {
            self.add_moves::<MT, CHECK>(s, rook_attacks(s, occ) & ok & self.state.pin_orth, ml);
        });
    }
}
