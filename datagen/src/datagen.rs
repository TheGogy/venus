...
const ADJ_RATIO: f32 = 0.8;
const WIN_ADJ_SCORE: Eval = Eval(2500);
const WIN_ADJ_PLIES: usize = 6;
...
const RANDOM_MOVES: usize = 8;
const VERIFICATION_NODES: u64 = 50000;
...
pub fn run_datagen(opts: DataGenOpts) -> anyhow::Result<()> {
    ...
    let run_id = format!("run_{}", chrono::Local::now().format("%Y-%m-%d_%H-%M"));
    ...
    std::thread::scope(|s| -> anyhow::Result<()> {
        ...
        for thread_id in 0..opts.threads {
            ...
            handles.push(s.spawn(move || thread_gen_data(thread_id, seed, outdir_ref, games_per_thread, tb, tc)));
        }
...
fn thread_gen_data(thread_id: usize, seed: u64, outdir: &Path, games: usize, tb: SyzygyTB, tc: TimeControl) -> anyhow::Result<()> {
    ...
    for g in 0..games {
        if thread_id == 0 && g % 64 == 0 {
            report_stats(g, games, start);
        }
        ...
        gen_random_position(&mut pos, &mut rng, RANDOM_MOVES);
...
fn report_stats(completed: usize, total: usize, start: Instant) {
    if completed == 0 {
        return;
    }
    ...
    println!(
        "GAMES={:<8} ({:>6.2}%) | ETA={:>15} | WW={:<7} ({:>5.2}%) | BW={:<7} ({:>5.2}%) | DR={:<7} ({:>5.2}%) | ADJ={:<7}",
        completed,
        progress,
        time_remaining,
        t_w,
