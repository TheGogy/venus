...
const MASK_AGE: u64 = 0x3F << SHIFT_AGE;
...
pub const fn pack_pv(was_pv: bool) -> u64 {
    was_pv as u64
}
...
pub const fn pack_eval(eval: i16) -> u64 {
    (eval as u16 as u64) << SHIFT_EVAL
}

pub const fn pack_value(value: i16) -> u64 {
    (value as u16 as u64) << SHIFT_VALUE
}

pub const fn pack_depth(depth: u8) -> u64 {
    (depth as u64) << SHIFT_DEPTH
}

pub const fn pack_move(mv: Move) -> u64 {
...
pub const fn unpack_depth(data: u64) -> u8 {
    ((data & MASK_DEPTH) >> SHIFT_DEPTH) as u8
}
...
pub const fn unpack_move(data: u64) -> Move {
    Move(((data & MASK_MOVE) >> SHIFT_MOVE) as u16)
}

pub const fn unpack_eval(data: u64) -> i16 {
    ((data & MASK_EVAL) >> SHIFT_EVAL) as i16
}

pub const fn unpack_value(data: u64) -> i16 {
    ((data & MASK_VALUE) >> SHIFT_VALUE) as i16
}
